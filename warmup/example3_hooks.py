#!/usr/bin/which python3
from keystone import *
from capstone import *
from unicorn import *
from unicorn.arm_const import *
#some address, where we store our code
ADDRESS = 0x1000
MEM_HOOK_ADDRESS = 0x80000
#First we define some assembly

def hook_code(uc, address, size, user_data):
    print(f"REG R1 is:{hex(uc.reg_read(UC_ARM_REG_R1))}")

def read_mem_hook():
    raise "not implemented"
def exception_hook():
    raise "not implemented"

CODE ='''
    mov r1,0x1;
    mov r2,1
    mov r4,0x4;
    orr r3,r1,r3
    '''
def main():
    #Now we tell what kind of assembler we want to use from keystone!
    ks = Ks(KS_ARCH_ARM,KS_MODE_ARM)
    encoding, count = ks.asm(CODE,as_bytes=True)
    #now we start build ourselves a unicorn engine!
    uc = Uc(UC_ARCH_ARM,UC_MODE_ARM)
    uc.mem_map(ADDRESS,2*1024*1024)
    uc.mem_write(ADDRESS, encoding)
    #we write some random value into a register
    uc.reg_write(UC_ARM_REG_R3,0x1337)
    uc.hook_add(UC_HOOK_CODE,hook_code)
    uc.emu_start(ADDRESS,ADDRESS+len(encoding),count=2000)
    print(f"<<<<PC = {hex(uc.reg_read(UC_ARM_REG_PC))}")
    print(f"<<<<R3 = {hex(uc.reg_read(UC_ARM_REG_R3))}")
    
if __name__ == "__main__":
    main()
